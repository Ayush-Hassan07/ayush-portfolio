...
export default async function RootLayout({ children }: { children: ReactNode }) { const profile = await getPublicProfile(); return <html lang="en"><body><AnalyticsTracker /><PublicShell name={profile?.name ?? "Ayush Hassan Raiyan"} email={profile?.email ?? null} githubUrl={profile?.github_url ?? null} linkedinUrl={profile?.linkedin_url ?? null} googleScholarUrl={profile?.google_scholar_url ?? null}>{children}</PublicShell></body></html>; }
