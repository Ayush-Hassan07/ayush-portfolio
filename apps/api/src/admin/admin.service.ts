...
export class AdminService {
  ...
  async listProjectMedia(slug: string) {
    const rows = await this.prisma.project_media.findMany({
      where: {
        project: { slug },
      },
      orderBy: [
    ...
    const media = await this.prisma.media_asset.findUnique({
      where: {
        storage_key: key,
      },
    });
  ...
  async replaceProjectMedia(slug: string, mediaIds: unknown) {
    if (
      ...
      throw new Error('mediaIds must be an array of ids');
    }
    ...
    await this.prisma.project_media.deleteMany({
      where: {
        project_id: project.id,
      },
    });
    ...
    if (mediaIds.length) {
      await this.prisma.project_media.createMany({
        data: mediaIds.map((media_id, index) => ({
          project_id: project.id,
  ...
  private async saveCertificationMedia(
    ...
    await this.prisma.certification_media.deleteMany({
      where: {
    ...
    if (value.length) {
      await this.prisma.certification_media.createMany({
        data: value.map((media_id, sort_order) => ({
          certification_id: certificationId,
          media_id: media_id as string,
          sort_order,
        })),
  ...
  async updateProfileImage(storageKey: string) {
    ...
    if (!profile) {
      throw new Error('Profile record does not exist');
    }
  ...
  async beginTotp(adminId: string) {
    ...
    return {
      ...
      otpauthUri:
        `otpauth://totp/Ayush%20Portfolio` +
        `?secret=${secret}` +
        `&issuer=Ayush%20Portfolio` +
        `&algorithm=SHA1` +
        `&digits=6` +
        `&period=30`,
    };
      ...
      image_url: (() => {
        ...
        if (typeof value === 'string') {
          try {
            ...
            /* keep relative or validated external URL */
          }
        }
